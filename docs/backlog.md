...
| Per-device sessions (log out one phone only) | Logout and password change bump `session_version`, which ends every device at once; accepted for the pilot. Waits until a clinic asks | `sessions` table keyed by device, JWT carries a session id, filter checks the row instead of the version |
| Login events | Proposed, not scaffolded. `login_events` tenant table: `practice_id`, `user_id`, `ip`, `outcome` (`success` \| `wrong_password`), `created_at`. Append-only with the same guard pattern as `patient_weights`. Successes and wrong-password attempts on a known user are recorded under that user's practice; unknown identifiers go to the application log only, since there is no practice to file them under. Written from `AuthService.login` inside `SystemContext` with an explicit `practice_id`, because there is no tenant before login | one migration (table, RLS, audit, export, `TenantIsolationIT` case), `AuthService.login`, a read endpoint on the user form |
| Token that survives a closed tab on the phone | Today the token is in `sessionStorage` and dies with the tab, so staff log in again after the phone kills the browser. Kept that way on purpose: a token in `localStorage` outlives the tab on a shared device. Revisit with a shorter token TTL plus a refresh token, or an idle timeout, so the longer life is bounded by something other than 12 h | `web/src/app/core/auth.service.ts`, `qvety.jwt.ttl`, a refresh endpoint |
| Patient photo upload | `patients.photo_object_key` and the MinIO container exist and stay unused until that part | upload endpoint, object key on the row, thumbnail on the patient card |
| Frontend unit tests for `list-page`, `form-page`, `viewport.service` | The Playwright walkthrough (`npm run e2e`) covers the screens end to end; component-level tests come with the first regression that the walkthrough misses | `web/src/app/shared/**/*.spec.ts`, `web/src/app/core/viewport.service.spec.ts` |
